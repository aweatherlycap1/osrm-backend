                                    ...
                                    const IntersectionType &intersection) const
{
    ...
    if (candidate_deviation < GROUP_ANGLE)
    {
        ...
        auto const is_similar_turn = [&](auto const &road) {
            ...
            if (opposing_to_compare_road_is_distinct && compare_road_deviation_is_distinct)
            {
                return false;
            }

            if (no_name_change_to_candidate && no_name_change_to_compare_from_opposing &&
            ...
            const auto candidate_road_has_same_priority_group =
                via_edge_data.flags.road_classification.GetPriority() ==
                candidate_data.flags.road_classification.GetPriority();
            const auto compare_road_has_different_priority_group =
            ...
            if (candidate_road_has_same_priority_group && compare_road_has_different_priority_group)
            {
                return false;
            }
